...
QByteArray parse(QString request) {
    ...
    // auth&login&password
    // getTask&'0 or 1 or 2'
    // checkTask& '0 or 1 or 2' & variant & answer
    // getUserStat&login
    ...
    /*else if (method == QString("getTask1")) {
        response = getTask1(data_client.at(0));
    }
    else if (method == QString("getTask2")) {
        response = getTask2(data_client.at(0));
    }*/
...
QByteArray reg(QString login, QString password,QString email) {
    QVariantMap response;
    //QString token = QString(QCryptographicHash::hash((login + password).toUtf8(), QCryptographicHash::Sha256).toHex());
