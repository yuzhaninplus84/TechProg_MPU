...
QString Result(std::vector<int>&a, std::vector<double>&b, std::vector<double>&c, std::vector<double>&d)//
{
    QString str;
    //str += QString::number(a[0]) + " +" + QString::number(b[0]) + "x +" + QString::number(c[0]) + "x^2 +" + QString::number(d[0]) + "x^3=0\n";
    str += QString::number(a[0]) + " " + QString::number(b[0]) + " " + QString::number(c[0]) + " " + QString::number(d[0]) + "; ";
    for (int i = 1; i < Number - 1; i++)
    {
        //str += QString::number(a[i]) + " + " + QString::number(b[i]) + "(x-" + i + ") + "
        //+ QString::number(c[i]) + "(x-" + i + ")^2 + " + QString::number(d[i]) + "(x-" + i + ")^3 = 0\n";
        str += QString::number(a[i]) + " " + QString::number(b[i]) + "," + QString::number(i) + " " +
               QString::number(c[i]) + "," + QString::number(i) + " " + QString::number(d[i]) + "," + QString::number(i) + "; ";
    }
    return str;
}
